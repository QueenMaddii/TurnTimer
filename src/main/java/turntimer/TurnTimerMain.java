...
import turntimer.util.GeneralUtils;
import turntimer.util.KeywordInfo;
...
@SpireInitializer
public class TurnTimerMain implements
    ...
    private static final String resourcesFolder = "turntimer";
    ...
    public TurnTimerMain() {
        BaseMod.subscribe(this); //This will make BaseMod trigger all the subscribers at their appropriate times.
        BaseMod.subscribe(new EndOfTurn());
    ...
    public void receivePostInitialize() {
        ...
        settingsPanel.addUIElement(new ModSlider("Turn Time", 1000,500, 60, "s", settingsPanel, ms -> {
            EndOfTurn.setTurnTime(Math.round(ms.value * ms.multiplier));
        }));
...
//                localizationPath(lang, "MonsterStrings.json"));
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(TurnTimerMain.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
